...
<div class="wrapper">
    ...
    <div class="page">
        ...
        <!-- .page-inner -->
        <form action="{{route('dashboard.customers.articles.store')}}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('POST')
            <div class="mb-3">
              <label >Main Title</label>  @error('title')<div class="alert alert-danger">{{ $message }}</div>@enderror
              <input type="text" class="form-control" name="title" class="@error('title') is-invalid @enderror">
            </div>
            <div class="mb-3">
              <label >Title</label>  @error('title2')<div class="alert alert-danger">{{ $message }}</div>@enderror
              <input type="text" class="form-control" name="title2" class="@error('title2') is-invalid @enderror">
            </div>
            <div class="mb-3"> 
